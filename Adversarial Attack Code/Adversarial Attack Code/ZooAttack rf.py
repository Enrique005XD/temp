...
df=pd.read_csv('Merged Dataset 1 (another copy).csv')
...
def get_adversarial_examples(x_train, y_train):
    ...
    zoo = ZooAttack(classifier=art_classifier, confidence=0.0, targeted=False, learning_rate=3e-1, max_iter=25,
...
model = RandomForestClassifier(n_estimator=100, max_depth=70,
				min_samples_leaf=10)
...
zoo = ZooAttack(classifier=art_classifier, confidence=50.0, targeted=False, learning_rate=3e-1, max_iter=25,
...
Accuracy = model.score(x_test_adv, y_test)
print("Adversarial Acc. Rate: %.4f" % Accuracy)
